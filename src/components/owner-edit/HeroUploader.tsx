 ...
 */
export default function HeroUploader({ heroUrl, onChange, slug }: Props) {
  ...
  async function onPick(e: ChangeEvent<HTMLInputElement>) {
    ...
    fd.append("file", file);
    fd.append("slug", slug); // multi-listing claimed_by ownership check (TDL #620)
  ...
  async function handleDelete() {
    setErr(null);
    const res = await fetch(`/api/photos/hero?slug=${encodeURIComponent(slug)}`, { method: "DELETE" });
