...
// NOTE (TDL #620): year_established + social_* are intentionally NOT listed —
// mortgage_listings has no such columns, so accepting them would error the
// UPDATE. The form omits them; trimming here is defensive (prevents a future
// form re-introducing them from silently breaking saves). The sanitizers and
// isValidSocial() below are retained (UrlInput still imports isValidSocial).
