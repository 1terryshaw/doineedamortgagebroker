...
import { VERTICAL_KEY } from "@/lib/claim-pitch";

export const dynamic = "force-dynamic";

// NEW-owner claim (TDL #624, ported from doineedanaccountant). Collects email +
// name on /claim/[slug], stamps owner_auth_token/owner_email/owner_name on the
// row, and emails a verify link. /api/claim/verify flips is_claimed on click.
export async function POST(request: NextRequest) {
  try {
    const { slug, email, name, pending_description, src, lid } = await request.json();
    ...
    // TDL #510: claim-abuse gate (free-email-vs-corporate + 72h cooling-off).
    // Shared RPC; fail-open on error so legit owners are never blocked.
    try {
    ...
    // Send the verification email FIRST — if it fails, don't leave an orphan
    // owner_auth_token / owner_email on the row.
    ...
    try {
      ...
      if (!result.ok) {
        console.error(JSON.stringify({ event: "claim_send_error", email_redacted: emailRedacted, slug, err: result.error }));
        return emailFailed;
      }
      console.log(JSON.stringify({ event: "claim_send_ok", email_redacted: emailRedacted, slug, resend_id: result.id }));
    } catch (emailErr) {
      console.error(JSON.stringify({ event: "claim_send_error", email_redacted: emailRedacted, slug, err: String(emailErr) }));
      return emailFailed;
    }
    ...
    if (typeof pending_description === "string" && pending_description.trim()) {
      update.pending_description = pending_description.trim().slice(0, 6000);
    }
    ...
    // TDL #472 lead-to-claim attribution (best-effort; never blocks the claim).
    if (src) {
      const cleanLid = typeof lid === "string" ? lid.replace(/^i-/, "") : null;
      await supabaseAdmin
        .from("claim_attribution")
        .insert({ lead_id: cleanLid || null, vertical: VERTICAL_KEY, listing_id: listing.id, src, slug })
