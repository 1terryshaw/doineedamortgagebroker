...
export async function POST(request: NextRequest) {
  ...
  if (kind === "photo") {
    const photoLimit = photoLimitForTier(
      (listing.tier as string | null) || (listing.subscription_tier as string | null)
    );
  ...
  const { data: row, error: insertError } = await supabaseAdmin
    ...
    .insert({
      ...
      uploaded_by_email: (listing.owner_email as string | null) ?? null,
    })
